...
final class Version00000000000000 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql(
            'CREATE TABLE product (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, priority INTEGER DEFAULT NULL, name VARCHAR(255) NOT NULL, price NUMERIC(10, 2) NOT NULL, color CHAR(7) DEFAULT NULL, icon VARCHAR(255) NOT NULL, image VARCHAR(255) DEFAULT NULL, category_id INTEGER DEFAULT NULL, CONSTRAINT FK_D34A04AD12469DE2 FOREIGN KEY (category_id) REFERENCES category (id) NOT DEFERRABLE INITIALLY IMMEDIATE)'
        );
        $this->addSql('CREATE INDEX IDX_D34A04AD12469DE2 ON product (category_id)');
        $this->addSql(
            'CREATE TABLE product_event (product_id INTEGER NOT NULL, event_id INTEGER NOT NULL, PRIMARY KEY(product_id, event_id), CONSTRAINT FK_9AF271FB4584665A FOREIGN KEY (product_id) REFERENCES product (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_9AF271FB71F7E88B FOREIGN KEY (event_id) REFERENCES event (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)'
        );
        $this->addSql('CREATE INDEX IDX_9AF271FB4584665A ON product_event (product_id)');
        $this->addSql('CREATE INDEX IDX_9AF271FB71F7E88B ON product_event (event_id)');
        $this->addSql(
            'CREATE TABLE purchase_transaction (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, transaction_id BLOB NOT NULL, payment_type VARCHAR(255) NOT NULL, created_at DATETIME NOT NULL, price NUMERIC(10, 2) NOT NULL, event_name VARCHAR(255) NOT NULL)'
        );
        $this->addSql('CREATE UNIQUE INDEX UNIQ_98A1D282FC0CB0F ON purchase_transaction (transaction_id)');
        $this->addSql('CREATE INDEX IDX_98A1D288B8E842841E832AD ON purchase_transaction (created_at, event_name)');
        $this->addSql(
        ...
        $this->addSql('CREATE INDEX IDX_F3D4A2464584665A ON purchased_article (product_id)');
